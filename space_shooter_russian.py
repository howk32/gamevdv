...
# Создаем полноэкранный дисплей
screen = pygame.display.set_mode((0, 0), pygame.FULLSCREEN)
# Получаем фактические размеры экрана
...
try:
    ...
    print("Не удалось загрузить Shoot.wav")

try:
    boom_sound = pygame.mixer.Sound("Boom.wav")
except:
    print("Не удалось загрузить Boom.wav")

try:
    menu_sound = pygame.mixer.Sound("menu.wav")
except:
    print("Не удалось загрузить menu.wav")
...
try:
    ship_image = pygame.image.load("da.png").convert_alpha()
except:
    print("Не удалось загрузить da.png")
...
for i in range(1, 6):  # Загружаем 1.png through 5.png
    try:
        ...
        enemy_ship_images.append(img)
    except:
        enemy_ship_images.append(None)
        print(f"Не удалось загрузить {i}.png")
...
class Player:
    ...
    def draw(self):
        if ship_image:
            # Уменьшаем изображение корабля
            scaled_ship = pygame.transform.scale(ship_image, (self.width, self.height))
            # Поворачиваем изображение корабля с регулируемым смещением
            rotated_ship = pygame.transform.rotate(scaled_ship, -(self.angle + self.image_rotation_offset))
...
class Enemy:
    ...
    def draw(self):
        # Используем изображение, зависящее от уровня, если доступно
        if self.level <= len(enemy_ship_images) and enemy_ship_images[self.level - 1]:
            # Уменьшаем изображение вражеского корабля
            scaled_ship = pygame.transform.scale(enemy_ship_images[self.level - 1], (self.width, self.height))
            # Поворачиваем изображение вражеского корабля с регулируемым смещением
            rotated_ship = pygame.transform.rotate(scaled_ship, -(self.angle + self.image_rotation_offset))
            ship_rect = rotated_ship.get_rect(center=(self.x, self.y))
            screen.blit(rotated_ship, ship_rect.topleft)
        else:
...
class Explosion:
    def __init__(self, x, y, explosion_type="small"):
        self.x = x
        self.y = y
        self.type = explosion_type  # "small" для boom.png, "large" для boom2.png
        self.life = 15  # Большое время отображения
        self.scale = 0.03  # Минимальный масштаб
        self.alpha = 30
    ...
    def update(self):
        ...
        # Эффект масштабирования для больших взрывов (уменьшен)
        if self.type == "large":
            self.scale = 0.03 + (15 - self.life) * 0.002
            
    def draw(self):
        # Резервный вариант к эффекту частиц, если загрузка изображения не удалась
        pygame.draw.circle(screen, YELLOW, (int(self.x), int(self.y)), 
                         max(1, int(10 * (self.life / 30))))
...
class Button:
    def __init__(self, x, y, width, height, text, color=BLUE, hover_color=CYAN):
        ...
        self.sound_played = False  # Чтобы звук не воспроизводился повторно
    ...
    def is_hovered(self, pos):
        if self.rect.collidepoint(pos):
            if self.current_color != self.hover_color and not self.sound_played:
                self.current_color = self.hover_color
                # Проигрываем звук меню при наведении на кнопку
                if menu_sound:
                    menu_sound.play()
    ...
    def is_clicked(self, pos, event):
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
            if self.rect.collidepoint(pos):
                # Проигрываем звук меню при нажатии на кнопку
                if menu_sound:
                    menu_sound.play()
                return True
        return False
...
def main():
    ...
    shop_button = Button(SCREEN_WIDTH//2 - 100, SCREEN_HEIGHT//2 + 70, 200, 50, "Магазин", BLUE, CYAN)
    quit_button = Button(SCREEN_WIDTH//2 - 100, SCREEN_HEIGHT//2 + 140, 200, 50, "Выход", BLUE, CYAN)
    
    # Создаем кнопки для режима игры
    menu_button = Button(SCREEN_WIDTH - 160, 20, 140, 50, "Меню", (60, 100, 200), (60, 150, 255))
    shop_game_button = Button(SCREEN_WIDTH - 160, 80, 140, 50, "Магазин", (60, 100, 200), (60, 150, 255))
    
    # Создаем кнопки магазина
    damage_button = Button(0, 0, 150, 40, "Урон +5", BLUE, CYAN)
    health_button = Button(0, 0, 150, 40, "Здоровье +20", BLUE, CYAN)
    speed_button = Button(0, 0, 150, 40, "Скорость +1", BLUE, CYAN)
    armor_button = Button(0, 0, 150, 40, "Броня +1", BLUE, CYAN)
    attack_speed_button = Button(0, 0, 150, 40, "Скорость атаки +1", BLUE, CYAN)
    back_button = Button(0, 0, 150, 40, "Назад", BLUE, CYAN)
    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_SPACE and game_state == PLAYING and player1:
                    ...
                    game_state = SHOP
                elif event.key == pygame.K_1 and game_state == SHOP and player1:
                    # Улучшаем урон
                    if score >= player1.upgrade_costs['damage']:
                        score -= player1.upgrade_costs['damage']
                        player1.upgrades['damage'] += 1
                        player1.damage += 5
                        player1.upgrade_costs['damage'] = int(player1.upgrade_costs['damage'] * 1.5)  # Увеличиваем стоимость
                elif event.key == pygame.K_2 and game_state == SHOP and player1:
                    # Улучшаем здоровье
                    if score >= player1.upgrade_costs['health']:
                        score -= player1.upgrade_costs['health']
                        player1.upgrades['health'] += 1
                        player1.max_health += 20
                        player1.health += 20
                        player1.upgrade_costs['health'] = int(player1.upgrade_costs['health'] * 1.5)  # Увеличиваем стоимость
                elif event.key == pygame.K_3 and game_state == SHOP and player1:
                    # Улучшаем скорость
                    if score >= player1.upgrade_costs['speed']:
                        score -= player1.upgrade_costs['speed']
                        player1.upgrades['speed'] += 1
                        player1.speed += 1  # Используем целочисленное увеличение
                        player1.upgrade_costs['speed'] = int(player1.upgrade_costs['speed'] * 1.5)  # Увеличиваем стоимость
                elif event.key == pygame.K_4 and game_state == SHOP and player1:
                    # Улучшаем броню
                    if score >= player1.upgrade_costs['armor']:
                        score -= player1.upgrade_costs['armor']
                        player1.upgrades['armor'] += 1
                        player1.armor += 1
                        player1.upgrade_costs['armor'] = int(player1.upgrade_costs['armor'] * 1.5)  # Увеличиваем стоимость
                ...
                if game_state == MENU:
                    if start_button.is_clicked(mouse_pos, event):
                        ...
                        # Создаем временного игрока для показа магазина
                        player1 = Player(1)
                        game_state = SHOP
                    ...
                    if menu_button.is_clicked(mouse_pos, event):
                        game_state = MENU
                    elif shop_game_button.is_clicked(mouse_pos, event):
                        game_state = SHOP
                elif game_state == SHOP:
                    # Обрабатываем нажатия кнопок магазина
                    if damage_button.is_clicked(mouse_pos, event) and player1:
                        # Улучшаем урон
                        cost = player1.upgrade_costs['damage']
                        if score >= cost:
                            score -= cost
                            player1.upgrades['damage'] += 1
                            player1.damage += 5
                            player1.upgrade_costs['damage'] = int(cost * 1.5)  # Увеличиваем стоимость
                            # Проигрываем звук меню
                            if menu_sound:
                                menu_sound.play()
                    elif health_button.is_clicked(mouse_pos, event) and player1:
                        # Улучшаем здоровье
                        cost = player1.upgrade_costs['health']
                        if score >= cost:
                            score -= cost
                            player1.upgrades['health'] += 1
                            player1.max_health += 20
                            player1.health += 20
                            player1.upgrade_costs['health'] = int(cost * 1.5)  # Увеличиваем стоимость
                            # Проигрываем звук меню
                            if menu_sound:
                                menu_sound.play()
                    elif speed_button.is_clicked(mouse_pos, event) and player1:
                        # Улучшаем скорость
                        cost = player1.upgrade_costs['speed']
                        if score >= cost:
                            score -= cost
                            player1.upgrades['speed'] += 1
                            player1.speed += 1
                            player1.upgrade_costs['speed'] = int(cost * 1.5)  # Увеличиваем стоимость
                            # Проигрываем звук меню
                            if menu_sound:
                                menu_sound.play()
                    elif armor_button.is_clicked(mouse_pos, event) and player1:
                        # Улучшаем броню
                        cost = player1.upgrade_costs['armor']
                        if score >= cost:
                            score -= cost
                            player1.upgrades['armor'] += 1
                            player1.armor += 1
                            player1.upgrade_costs['armor'] = int(cost * 1.5)  # Увеличиваем стоимость
                            # Проигрываем звук меню
                            if menu_sound:
                                menu_sound.play()
                    elif attack_speed_button.is_clicked(mouse_pos, event) and player1:
                        # Улучшаем скорость атаки (уменьшаем задержку)
                        cost = player1.upgrade_costs.get('attack_speed', 100)
                        if score >= cost:
                            score -= cost
                            if 'attack_speed' not in player1.upgrades:
                                player1.upgrades['attack_speed'] = 0
                            player1.upgrades['attack_speed'] += 1
                            # Уменьшаем задержку на 2 (более быстрая стрельба)
                            player1.shoot_cooldown = max(5, player1.shoot_cooldown - 2)
                            player1.upgrade_costs['attack_speed'] = int(cost * 1.5)  # Увеличиваем стоимость
                            # Проигрываем звук меню
                            if menu_sound:
                                menu_sound.play()
        ...
        if game_state == MENU:
            ...
            shop_button.is_hovered(mouse_pos)
        ...
        elif game_state == SHOP and player1:
            # Обновляем состояния наведения кнопок
            damage_button.is_hovered(mouse_pos)
            health_button.is_hovered(mouse_pos)
            speed_button.is_hovered(mouse_pos)
            armor_button.is_hovered(mouse_pos)
            attack_speed_button.is_hovered(mouse_pos)
            back_button.is_hovered(mouse_pos)
            
            # Рисуем интерфейс магазина
            ...
            # Заголовок магазина
            shop_title = font_large.render("МАГАЗИН УЛУЧШЕНИЙ", True, YELLOW)
            ...
            stats = [
                f"Очки: {score}",
                f"Урон: {player1.damage} (Уровень {player1.upgrades['damage']})",
                f"Здоровье: {player1.health}/{player1.max_health} (Уровень {player1.upgrades['health']})",
                f"Скорость: {player1.speed} (Уровень {player1.upgrades['speed']})",
                f"Броня: {player1.armor} (Уровень {player1.upgrades['armor']})"
            ]
            
            for i, stat in enumerate(stats):
                stat_text = font_small.render(stat, True, WHITE)
                screen.blit(stat_text, (SCREEN_WIDTH // 2 - stat_text.get_width() // 2, stats_y + i * 40))
            
            # Позиционируем кнопки магазина
            damage_button.rect.center = (SCREEN_WIDTH//2 - 200, SCREEN_HEIGHT//2)
            health_button.rect.center = (SCREEN_WIDTH//2 + 200, SCREEN_HEIGHT//2)
            speed_button.rect.center = (SCREEN_WIDTH//2 - 200, SCREEN_HEIGHT//2 + 60)
            armor_button.rect.center = (SCREEN_WIDTH//2 + 200, SCREEN_HEIGHT//2 + 60)
            attack_speed_button.rect.center = (SCREEN_WIDTH//2, SCREEN_HEIGHT//2 + 120)
            back_button.rect.center = (SCREEN_WIDTH//2, SCREEN_HEIGHT//2 + 180)
            
            # Рисуем кнопки магазина
            damage_button.draw()
            health_button.draw()
            speed_button.draw()
            armor_button.draw()
            attack_speed_button.draw()
            back_button.draw()
            ...
            shop_text = font_small.render("B для магазина, ESC для меню", True, LIGHT_GRAY)
            ...
            shop_game_button.is_hovered(mouse_pos)
                ...
                for bullet in player1.bullets[:]:
                    ...
                    if enemy.health <= 0:
                        if current_level >= max_levels:
                            ...
                            # Добавляем большой взрыв при уничтожении врага
                            explosions.append(Explosion(enemy.x, enemy.y, "large"))
                            # Проигрываем звук взрыва для большого взрыва
                            if boom_sound:
                                boom_sound.play()
                ...
                for bullet in enemy.bullets[:]:
                    ...
                    if bullet.get_rect().colliderect(player1.get_rect()):
                        ...
                        if player1.health <= 0:
                            ...
                            # Добавляем большой взрыв при уничтожении игрока
                            explosions.append(Explosion(player1.x, player1.y, "large"))
                            # Проигрываем звук взрыва для большого взрыва
                            if boom_sound:
                                boom_sound.play()
            ...
            if player1 and enemy and hasattr(player1, 'get_rect') and hasattr(enemy, 'get_rect') and player1.get_rect().colliderect(enemy.get_rect()):
                player1.health -= 1
                # Добавляем взрыв в точке столкновения
                collision_x = (player1.x + enemy.x) / 2
                collision_y = (player1.y + enemy.y) / 2
                explosions.append(Explosion(collision_x, collision_y, "small"))
                # Проигрываем звук взрыва
                if boom_sound:
                    boom_sound.play()
                # Одиночный режим - игра окончена, когда умирает игрок
